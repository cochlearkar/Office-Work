...
// ── WhatsApp URL builder — standard WA Messenger only, opens chat ──────────
// On Android: uses intent:// to force com.whatsapp (not Business)
// On iOS: uses wa.me (user controls which app handles it in Settings)
function _waUrl(phone) {
  const digits = phone.replace(/\D/g, "");
  const isAndroid = /android/i.test(navigator.userAgent);
  if (isAndroid) {
    return `intent://send/${digits}#Intent;scheme=whatsapp;package=com.whatsapp;end`;
  }
...
export function groupCallCard(gc) {
  ...
  const contactRows = contacts.map((c, i) => {
    ...
    const waBtn    = c.phone ? `<a class="gcc-wa-btn" href="${_waUrl(c.phone)}" title="Open in WhatsApp Messenger">
